...
void StateS::loadMachine(const QString& path)
{
	...
	if ( (file.exists()) && ( (file.permissions() & QFileDevice::ReadUser) != 0) )
	{
		...
		if (parser == nullptr)
		{
			...
			errorToDisplay.append(tr("The file may also be a correct StateS file but saved with a version prior to 0.5.0."));
			errorToDisplay.append(tr("Files from versions earlier than 0.5.0 are not compatible with current StateS version."));
